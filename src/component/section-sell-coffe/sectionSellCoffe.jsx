...
export function SectionSellCoffe({ id, title, children , action }) {
  ...
  return (
    <div className="section-sell-item">
      <div onClick={()=>dispatch(action())} className="button-title-arrow">
        <strong> {title}</strong>
        <button >
          <img className={`icon_arrow_up ${OpenCard.bol ? 'active':''}`} src={icon_arrow_up} alt="" />
          <img className={`icon_arrow_down ${OpenCard.bol ? '':'active'} `} src={icon_arrow_down} alt="" />
        </button>
      </div>
